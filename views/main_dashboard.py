...
def render_main_dashboard():
    """
    عرض الوقت والتاريخ الحالي مع مؤشرات الطقس الدائرية وحالة النظام.
    """
    st.markdown(
        """
        <style>
        ...
        .external-label { color: #64748B !important; font-size: 12px; font-weight: 600; }
    ...
    # 2. مؤشرات الطقس الدائرية المبسطة بكلمة تعريفية واحدة
    ...
    with w_col1:
        st.markdown('<div class="ring-card-container">', unsafe_allow_html=True)
        t_color = "#EA580C" if mock_temperature >= 40.0 else "#1D4ED8"
        fig_temp = create_sovereign_gauge(mock_temperature, 60.0, "°C", t_color)
        st.plotly_chart(fig_temp, use_container_width=False, config={"displayModeBar": False}, key="gauge_temp_plot")
        st.markdown(f'<div class="ring-card-label">{get_text("Temperature")}</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
        
    with w_col2:
        st.markdown('<div class="ring-card-container">', unsafe_allow_html=True)
        fig_hum = create_sovereign_gauge(mock_humidity, 100.0, "%", "#06B6D4")
        st.plotly_chart(fig_hum, use_container_width=False, config={"displayModeBar": False}, key="gauge_hum_plot")
        st.markdown(f'<div class="ring-card-label">{get_text("Humidity")}</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
        
    with w_col3:
        st.markdown('<div class="ring-card-container">', unsafe_allow_html=True)
        fig_wind = create_sovereign_gauge(mock_wind_speed, 50.0, " km/h", "#10B981")
        st.plotly_chart(fig_wind, use_container_width=False, config={"displayModeBar": False}, key="gauge_wind_plot")
        st.markdown(f'<div class="ring-card-label">{get_text("Wind Speed")}</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
